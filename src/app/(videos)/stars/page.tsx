...
import { Search, Sparkles, MoveRight, Star, Users } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <section className="relative overflow-hidden pt-16 pb-16 md:pt-32 md:pb-32">
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6 text-center">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 rounded-full border border-violet-200/50 bg-violet-50/50 px-4 py-1.5 text-sm font-medium text-violet-700 backdrop-blur-sm dark:border-violet-800/50 dark:bg-violet-950/30 dark:text-violet-300"
          >
          ...
          <motion.h1 
            ...
            className="mt-6 md:mt-8 text-4xl font-extrabold tracking-tight sm:text-6xl md:text-7xl"
          >
          ...
          <motion.p 
            ...
            className="mx-auto mt-4 md:mt-6 max-w-2xl text-base md:text-lg text-muted-foreground/80 sm:text-xl"
          >
          ...
          <motion.div 
            ...
            className="mx-auto mt-8 md:mt-12 w-full max-w-lg"
          >
            <form onSubmit={handleSearch} className="group relative">
              ...
              <div className="relative flex items-center overflow-hidden rounded-2xl border bg-background/80 shadow-2xl backdrop-blur-xl transition-all focus-within:ring-2 focus-within:ring-violet-500/50 dark:bg-background/60">
                ...
                <Input
                  ...
                  className="h-14 border-none bg-transparent px-4 text-base md:text-lg placeholder:text-muted-foreground/50 focus-visible:ring-0"
                />
                <Button
                  type="submit"
                  size="lg"
                  className="m-1 h-12 min-w-[44px] rounded-xl bg-foreground text-background hover:bg-foreground/90"
                >
                  Search
                </Button>
              </div>
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 pb-24">
        {activeSearch && (
          <div className="mb-8 md:mb-10 flex items-center justify-between border-b pb-4">
            <p className="text-base md:text-lg font-medium">
              &quot;{activeSearch}&quot; 검색 결과
            </p>
            <button
              onClick={() => { setActiveSearch(""); setSearch(""); setPage(1); }}
              className="min-h-[44px] min-w-[44px] text-sm text-muted-foreground hover:text-foreground hover:underline"
            >
              전체 보기
            </button>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-[400px] flex-col items-center justify-center rounded-3xl border border-dashed bg-muted/30 p-6 text-center">
          ...
          <motion.div 
            ...
            className="grid grid-cols-2 gap-x-4 gap-y-8 sm:gap-x-8 sm:gap-y-16 lg:grid-cols-3 xl:grid-cols-4"
          >
            {stars.map((star) => (
              <motion.div
                key={star.id}
        ...
        {data && data.totalPages > 1 && (
          <div className="mt-16 md:mt-20 flex justify-center gap-2 md:gap-4">
            <Button 
              variant="outline" 
              disabled={page <= 1} 
            ...
            <div className="flex items-center px-2 md:px-4 font-mono text-sm">
              {page} <span className="mx-1 md:mx-2 text-muted-foreground">/</span> {data.totalPages}
            </div>
            <Button 
              variant="outline" 
  ...
  return (
    <Link href={`/stars/profile/${star.id}`} className="group relative block">
      ...
      <div className="relative aspect-[3.5/4.5] w-full overflow-hidden rounded-3xl bg-muted transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-xl group-hover:shadow-violet-500/20 dark:group-hover:shadow-violet-900/40">
        ...
        <div className="absolute bottom-0 left-0 p-4 md:p-6 opacity-0 transition-all duration-300 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0">
          <p className="line-clamp-3 text-xs md:text-sm font-medium leading-relaxed text-white/90">
            {star.bio || "No bio available."}
          </p>
          <div className="mt-3 md:mt-4 flex items-center gap-2 text-xs font-semibold text-amber-400">
      ...
      <div className="mt-4 md:mt-5 space-y-1">
        <h3 className="text-lg md:text-xl font-bold tracking-tight text-foreground transition-colors group-hover:text-violet-600 dark:group-hover:text-violet-400">
          {star.name}
        </h3>
        <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm font-medium text-muted-foreground">
          <span>{star.videoCount} Videos</span>
          {star.followerCount !== undefined && (
            <>
