...
export async function PATCH(
  ...
  const target = await prisma.user.findUnique({ where: { id }, select: { id: true, canDirectUpload: true } });
  if (!target) return NextResponse.json({ error: { code: "NOT_FOUND", message: "사용자를 찾을 수 없습니다." } }, { status: 404 });

  const updated = await prisma.user.update({
    where: { id },
    data: { canDirectUpload: !target.canDirectUpload },
    select: { id: true, canDirectUpload: true },
  });
