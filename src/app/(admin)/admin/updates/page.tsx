    ...
    const updates = [
        {
            ...
            title: "정산 항목 상세 정보 개선",
            items: [
                "작품 이름 표시: 정산 항목에 영상 제목이 표시되어 어떤 작품에 대한 정산인지 한눈에 확인할 수 있습니다.",
                "클릭하여 상세 보기: 정산 항목을 클릭하면 해당 제출물의 피드백 페이지가 새 탭으로 열립니다.",
            ]
        },
        {
            version: "Bug Fix",
            title: "정산 총액 0원 표시 버그 수정",
            items: [
                "등급(Grade) 단가 반영: 기존에는 STAR 개인 단가만 사용하여, 등급 단가만 설정된 경우 총액이 0원으로 표시되던 문제를 수정했습니다. 이제 개인 단가 → 등급 단가 순으로 자동 적용됩니다.",
                "기존 0원 정산 재생성: 이미 생성된 0원 정산은 '정산 생성' 버튼으로 해당 월을 다시 생성하면 올바른 금액으로 갱신됩니다.",
            ]
        },
        {
            version: "Performance Update",
            title: "메인 페이지 로딩 속도 대폭 개선",
            items: [
                "서버 사이드 썸네일 URL 캐싱: 영상 목록 조회 시 매번 외부 API를 호출하던 썸네일 서명 로직에 메모리 캐시(50분 TTL)를 도입하여, 반복 요청 시 외부 호출 없이 즉시 응답합니다.",
                "클라이언트 캐싱 최적화: 불필요한 no-cache 설정을 제거하고 30초간 데이터를 재사용하도록 개선하여, 필터 변경 및 페이지 전환 시 체감 속도가 크게 향상되었습니다.",
            ]
        },
        {
            version: "Feature Update",
