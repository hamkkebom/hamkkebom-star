                ...
                <div className="flex items-end justify-between">
                    <div className="flex-1 min-w-0 pr-4">
                        <div className="flex items-center gap-2 mb-3">
                            <Avatar className="w-10 h-10 border-2 border-white/20 shadow-lg pointer-events-auto">
                                <AvatarImage src={(item.star as any).avatarUrl || undefined} />
                                <AvatarFallback className="bg-slate-800 text-white font-bold">{item.star.name[0]}</AvatarFallback>
                            </Avatar>
                            <div>
                                <div className="text-white font-bold shadow-black drop-shadow-md">
                        ...
                        <h3 className="text-white font-black text-xl leading-snug mb-2 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] truncate whitespace-normal line-clamp-2">
                            {projectTitle}
                        </h3>
                        {item.assignment?.request?.title && item.versionTitle && (
                            <div className="text-white/90 text-sm mb-3 drop-shadow-md line-clamp-1">
                                {item.assignment.request.title}
                            </div>
                        )}
                        <div className="flex flex-wrap gap-2 mt-2 pointer-events-auto">
                            <Badge className="bg-indigo-500/90 text-white border-white/10 backdrop-blur-md px-2.5 py-1">v{item.version.replace(/^v/i, "")}</Badge>
                            <Badge className="bg-black/60 text-white/90 border-white/20 backdrop-blur-md px-2.5 py-1">{statusLabels[item.status] || item.status}</Badge>
                            {item._count.feedbacks > 0 && <Badge className="bg-rose-500/90 text-white border-rose-400/30 backdrop-blur-md px-2.5 py-1">{item._count.feedbacks}개의 피드백</Badge>}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-5 items-center pointer-events-auto pl-2 pb-2">
                        <button
                            className="w-12 h-12 rounded-full bg-black/40 backdrop-blur-xl border border-white/20 flex flex-col items-center justify-center text-white hover:bg-white/20 transition-all active:scale-95 shadow-[0_8px_30px_rgba(0,0,0,0.5)] group"
