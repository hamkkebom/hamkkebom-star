            ...
            {filteredData.map((submission) => {
              ...
              return (
                <div
                  ...
                  className="group relative cursor-pointer flex overflow-hidden rounded-2xl border bg-card transition-all duration-200 active:scale-[0.98] active:bg-accent/50"
                >
                  {/* 썸네일 (좌측 고정 크기) */}
                  <div className="relative w-[140px] min-h-[90px] shrink-0 overflow-hidden bg-muted">
                    {submission.signedThumbnailUrl ? (
                      ...
                      <div className="h-full w-full bg-gradient-to-br from-indigo-500/5 via-purple-500/5 to-pink-500/5 flex items-center justify-center">
                    ...
                    {/* 영상 길이 (우측 하단) */}
                    {submission.duration && (
                      <div className="absolute bottom-1.5 right-1.5 bg-black/70 backdrop-blur-sm text-white text-[10px] px-1.5 py-0.5 rounded font-medium">
                  ...
                  <div className="flex flex-col flex-1 min-w-0 p-3 gap-1.5">
                    ...
                    <div className="mt-auto flex items-center gap-2 flex-wrap">
                      ...
                      {submission._count.feedbacks > 0 && (
                        <span className="flex items-center gap-1 font-medium text-indigo-500 text-[10px] ml-auto">
          ...
          <div className="hidden sm:grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredData.map((submission) => (
              <div
                key={submission.id}
                onClick={() => router.push(`/stars/my-videos/${submission.id}`)}
                className="group relative cursor-pointer flex flex-col overflow-hidden rounded-2xl border bg-card transition-all duration-300 hover:shadow-xl hover:-translate-y-1 active:scale-[0.98]"
              >
                {/* 썸네일 영역 */}
                <div className="relative aspect-video w-full overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-3 left-3 z-10">
                    {(() => {
                      ...
                      return (
                        <div className={cn(
                          "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold tracking-wide backdrop-blur-md shadow-lg",
                          statusInfo.className
                        )}>
